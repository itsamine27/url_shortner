#![deny(clippy::all)]                    // Catch serious issues
#![deny(clippy::pedantic)]              // Enforce good practices, naming, etc.

#![allow(clippy::missing_docs_in_private_items)] // No need to document everything during dev
#![allow(clippy::arbitrary_source_item_ordering)] // Ignore annoying import/mod sorting
#![allow(clippy::question_mark_used)]   // `?` is fine in idiomatic Rust
#![allow(clippy::str_to_string)]        // `"hello".to_string()` is okay
#![allow(clippy::absolute_paths)]       // `std::process::exit` is readable and fine
#![allow(clippy::module_name_repetitions)] // Avoid nags about function or struct names
...
async fn main() {
    ...
    serve(connect, app).await.unwrap_or_else(|err| debug!("internel server error {err}"));
}
#[derive(Deserialize)]
struct Qdata{
    name: Option<String>,
...
async fn shorten_url(State(mc): State<ModelController>, Json(data): Json<Formurl>) -> Result<impl IntoResponse, Response>{
...
async fn fetchurl(Path(url): Path<String>, State(mc): State<ModelController>) -> impl IntoResponse {
    (mc.db.fetchurl(url).await).map_or_else(|_| (StatusCode::NOT_FOUND, "Short URL not found").into_response(), |long_url| Redirect::temporary(&long_url).into_response())
}